...
## Step 25 — Transactional Outbox (Basket → Ordering) with the Outbox Pattern
Basket checkout now hands off to the new Ordering module reliably, solving the dual-write problem: `CheckoutBasketHandler` no longer publishes to RabbitMQ directly (the naive version is left commented for contrast). Instead it deletes the cart **and** writes a serialized `BasketCheckoutIntegrationEvent` row into a new `OutboxMessage` table in the same `SaveChanges` — one atomic DB transaction, so the intent-to-publish can never be lost or half-committed. A separate `OutboxProcessor : BackgroundService` (a singleton that creates its own DI scope per loop to get a short-lived scoped `BasketDbContext`) polls the table every 10 seconds, publishes any `ProcessedOn == null` rows via `IBus`, stamps `ProcessedOn`, and loops until the host cancels its `stoppingToken` on shutdown. On the receiving side `BasketCheckoutIntegrationEventHandler : IConsumer<...>` (auto-discovered by MassTransit) maps the event to a `CreateOrderCommand` and creates the order — delivery is at-least-once, so eventual consistency, not instant. New pieces: `Shared.Messaging/Events/BasketCheckoutIntegrationEvent`, `Basket/Models/OutboxMessage` + DbSet + migration, `Basket/Data/Processors/OutboxProcessor`, the `CheckoutBasket` feature, and the whole `Ordering` module (`Order`/`OrderItem` aggregates, `CreateOrder`, event handlers). Known naive spots for later steps: fixed 10s polling (latency + idle DB load; mature = `LISTEN/NOTIFY`/CDC), no consumer-side idempotency/inbox yet (duplicates possible), and hardcoded `OrderItem`s in the consumer because the integration event doesn't yet carry the cart's line items.
